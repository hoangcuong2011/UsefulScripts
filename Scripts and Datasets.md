...
In many Machine Learning problems, we usually have a linear model where features are constructed as strings. Let's take a look at the maxent model for pos tagging. For simplicty we assume we have a sequence of 5 words
where we already know the tag of the first two words

w_{i-2}/tag1, w_{i-1}/tag2, w_{i}/????, w_{i+1}/????, w_{i+2}}/????

Now our goal is to predict the tag for the last three words. 

Regarding to the problem of predicting the tag for word w_i, let us define a history (it should be a context, but people use the word history more often, actually) as:

history: {w_i, w_{i-1}, w_{i-2}, w_{i+1}, w_{i+2}, tag1, tag2]}

Here, a feature f(history) is something like this:

f_1(history) = 1 if suffix(w_i) = [ING], and 0 otherwise.

or something like this:

f_2(history) = 1 if word(w_{i-2}}) = I and 0 otherwise.

or 
f_3(history) = 1 if tag_{w_{i-2}} = NN and 0 otherwise.

Those features are arbitrary, and are generated from the training data based on some feature templates. The most basic ones can be follows from this classic paper: http://www.aclweb.org/anthology/W96-0213

For this kind of model we might generate hundreds of thousands, if not millions of features. (The number of features depend on the size of the training dataset). Those are sometimes called as sparse features, as they occur very sparsely in the training dataset. 

In the end, given a history, we want to classify which class it should belong to (This is equivalent to that given a word with its surrounding context, we want to tag the type of the word). 

How to ``represent" the history? Here is the thing. Machine learning algorithms, however, are typically defined in terms of numerical vectors. A very natural question is how to ``vectorize" those "text"-related features into vectors. A very naive is as follows: (See this: https://larsmans.github.io/ilps-hashing-trick/#slide4). First, let us assign a unique ID (i.e. a number) to each feature as follows

feature_to_index = {}
i = 0
for all features
	if feature not exist in the index:
		feature_to_index[f] = i
	    i += 1

Then we can construct (sparse) vectors with the size equals to the length N of feature_to_index
for each history:
	initialize a zero vector with size N
    for all features
        vector[feature_to_index[feature]] += 1

In the end, we can represent each feature as a vector with length N. Building a classifcation is then quite straightforward once we vectorize all these features and construct vectors for all history in the training data. The training dataformat is something like this:
...
The approach has at least 2 important drawbacks. The first drawback is that the length N could be too large to handle (i.e. memory issue as well speed issue). The second drawback is that it does not handle features that are not seen in the training data. For instance, let's say we build an email spam classifier. A trained model might be able to learn that words like Viagra is an important indicator that an email contains the word might be an spam email. However, let's us assume V1agra is a word that is not seen in the training data. Because of this, there is no way that our approach can detect V1agra could be similar to Viagra.


Technicallly, the first issue alone can be fixed by a better engineering skill. For instance, by using csr_matrix in Sklearn, we are able to compress the memory used to train the model at least 30% for common models (e.g. Naive Bayes, SVMs, Logistic Regressions). Computation time can be also improved a lot. See this for a reference: https://dziganto.github.io/Sparse-Matrices-For-Efficient-Machine-Learning/ )

But a more thorough solution is to use Feature hashing (https://en.wikipedia.org/wiki/Feature_hashing).
The idea is to use a hash function from strings to integers between 0 and pre-defined size  (normally around 10-90). Instead of maintaining an associate array (i.e. feature_to_index), we can use the hash values for features (as in the string format) directly as feature indices and updating the resulting vector at those indices. Pseudo-code is as follows:
...
Let's take an example from https://en.wikipedia.org/wiki/Feature_hashing. If our feature vector is ["cat","dog","cat"] and hash function is hash(cat)=1 and hash(dog)=2. Let us take the output feature vector dimension (N) to be 4. Then output x will be [0,2,1,0].

Working with hashing, collision is always an issue. A not perfect, but extremely smart solution is to use an extra single-bit output hash function ξ to determine the sign of the update value.
...
Experiments with various classification models are done in this paper https://alliance.seas.upenn.edu/~nlp/publications/pdf/ganchev2008.pdf. While the number of features is reduced by a factor of 10, they still obtain between 96.7% and 97.4% of the performance of the original
model, depending on the learning algorithm. As you can see, the algorithm yields substantial reductions in memory requirements with a minimal performance loss, a promising result for resource constrained devices.




Some open questions:
1. What is the requirement of a hash function? Does it need to be good, or Does it even need to be ``intelligent?" (e.g. to give the same hash value for both Viagra and V1agra).
Ideally, it should be the case, but perhaps it is too difficult to require having such a hash function.

2. Let's say we have two types of features:
word=bla bla bla

and 
tag = bla bla bla

Should we use the hash function on both the types of features to vectorize those vectors into a predefined N length format? Or should we use the hash function on each type of features (word=bla bla bla and tag = bla bla bla) seperately (for each one we vectorize them into a predefined N/2 length format).
...
D = [{'dog': 1, 'dog': 2, 'cat':2, 'elephant':4}, {'cat':2, 'dog': 1, 'elephant':4}, {'dog': 1, 'cat':2, 'elephant':4}]
f = h.transform(D)
print(f.toarray())
...
h = FeatureHasher(n_features=10)
X_new = h.fit_transform([{'dog': 1, 'cat':2, 'elephant':4}, {'cat':2, 'dog': 1, 'elephant':4}])
print(X_new.toarray())

print("cuong")

h = FeatureHasher(n_features=10, input_type='string')
X_new = h.fit_transform([{'dog', 'cat', 'elephant'}, {'dog', 'cat', 'cat', 'elephant'}])
print(X_new.toarray())


h = FeatureHasher(n_features=10, input_type='string')
X_new = h.fit_transform([{'w=dog', 'w=cat', 'w=elephant'}, {'w=dog', 'w=cat', 'w=cat', 'w=elephant'}])
print(X_new.toarray())
